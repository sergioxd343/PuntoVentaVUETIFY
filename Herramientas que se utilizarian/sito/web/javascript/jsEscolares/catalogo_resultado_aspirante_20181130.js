...
CatalogoResultadoAspirante.prototype.fnBuscar = function()
{
	...
	$.ajax({
        ...
        beforeSend:function(){
			...
			if(data[0].estatus === "EN LISTA DE ESPERA ")
			{
				cadena = '<p><strong>DATOS GENERALES</strong><br>'+
                        'Folio de Admisión: &nbsp;<strong>'+data[0].folio+'</strong><br>'+
                        'Nombre: &nbsp;<b>'+data[0].aspirante.trim()+'</b><br>'+
...
{	$('#xDivAceptado').hide();
	$('#xDivBtnDoc').hide();
	$('#gEncuesta').empty();
	$('#gEncuesta').load('../referencias/proceso_generar_referencia_ingreso.jsp',{xMatricula:$('#xTxtFolio').val(),xTurno:objProcesoResultadoAspirante.turno,xMonto:objProcesoResultadoAspirante.monto,xFechaLimite:objProcesoResultadoAspirante.fechaLimite,xFechaInscripcion:objProcesoResultadoAspirante.fechaInscripcion});
