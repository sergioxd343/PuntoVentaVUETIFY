...
var dialogF;
	...
	function Docente_Buscar(control, cve_unidad){

		dialogF.dialog("open");
		$jq("#o_ImgCarga").show();
		$jq("#contenido_pagina").attr('src', 'muestra_docente_frances.jsp?xControl=' + control +'&xUnidad=' + cve_unidad).load(function(){$jq("#o_ImgCarga").hide();});
	}
	...
	function Guardar(grupo)
		{
			...
			$jq.ajax({
			url: "ajax/proceso_datos_grupo_frances.jsp?yDocente="+docente+"&yMateria="+materia+"&yAula="+aula+"&yHorario="+horario+"&yGrupo="+grupo,
			type: "post",
			dataType: "html",
			cache: false,
			contentType: false,
		...
		function Actualizar(cve_captura)
		{
			...
			$jq.ajax({
			url: "ajax/proceso_captura_tardia.jsp?yAccion=Actualizar&yFecha="+fecha+"&yMotivo="+motivo+"&yCve_captura="+cve_captura,
			type: "post",
			dataType: "html",
			cache: false,
			contentType: false,
			...
			complete: function(){
				setTimeout(function(){$jq.dMsgUp("close");}, 3000);
				setTimeout(function(){Captura_Tardia_Nuevo();},4000);
			}
		});
		}

		function Eliminar(cve_captura)
		{
			$jq.ajax({
			url: "ajax/proceso_captura_tardia.jsp?yAccion=Eliminar&yCve_captura="+cve_captura,
			type: "post",
			dataType: "html",
			cache: false,
			contentType: false,
		...
		function Guardar(contador, matricula, grupo_a)
		{
			var grupo=document.getElementById("ycve_grupo_materia_frances").value;
			var fecha = document.getElementById("xFecha").value;
			var no_parcial = document.getElementById("xNoParcial").value;
			var motivo = document.getElementById("xMotivo").value;

			//alert("Grupo " + grupo)

			$jq.ajax({
			url: "ajax/proceso_captura_tardia.jsp?yAccion=Guardar&yCve_grupo="+grupo+"&yFecha="+fecha+"&yNo_Parcial="+no_parcial+"&yMotivo="+motivo,
			type: "post",
			dataType: "html",
			cache: false,
			contentType: false,
