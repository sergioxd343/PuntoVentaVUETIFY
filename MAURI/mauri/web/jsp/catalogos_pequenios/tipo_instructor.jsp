                    ...
                    <v-card>
                        ...
                        <v-container fluid>
                            ...
                            <v-row justify="center" class="align-center" style="padding: 0px 50px 0px 50px">
                                ...
                                <v-col md=4>
                                    <v-text-field 
                                        ...
                                        :error-messages="errors.first('nombre tipo instructor')"></v-text-field>
                                </v-col>

                                <v-row justify="center">
                                    <v-btn color="primary" @click="flagEditar ? fnEditar() : fnGuardarTipoInstructor()"><v-icon>mdi-content-save</v-icon>{{flagEditar ? 'Editar' : 'Guardar'}}</v-btn>
                                    &nbsp;
                            ...
                            <v-row justify="center">
                                <v-col md=12>
                                    <v-data-table
                                        :headers="headersTipoInstructor"
                                        :items="dataTipoInstructor"
                                        :search="searchTipos"
                                        class="elevation-2"
                                        no-data-text="No se encontro ningun registro"
                                        :hide-default-header="dataTipoInstructor.length < 1"
                                        :hide-default-footer="dataTipoInstructor.length < 1"
                                        locale="es-ES"
                                    ...
                                    <template v-slot:item.eliminar="{item}">
                                        <v-btn  small color="grey" @click="fnEliminarInstructor(item);" :disabled="!item.activo" >Inactivo</v-btn>
                                    </template>
    ...
    <script type="module">
        ...
        new Vue({
            ...
            setup(){
                ...
                const ctr = "../../controlador/catalogos_pequenios/Controlador_tipo_instructor.jsp";
                //Variables POST
                
                const nombreTipoInstructor = ref ("");
                ...
                const dataTipoInstructor = ref([]);
                

                const headersTipoInstructor = ref([
                ...
                onMounted(() => {                
                    fnConsultarTablaTipoInstructor();
                    
                });
                
               async function fnConsultarTablaTipoInstructor(){
                    try{
                        preloader("../../");
                ...
                async function fnGuardarTipoInstructor(){
                    this.$validator.validate().then(async esValido => {
                        if(esValido){
                            try{
                                ...
                                parametros.append("nombre_tipo_instructor", nombreTipoInstructor.value);
                                let {data,status} = await axios.post(ctr, parametros)
                                if(status == 200){
                                    if(data == "1"){
              ...
              async function fnEliminarInstructor(item){
                    
                            try{
                                preloader("../../");
                                let parametros = new URLSearchParams();
                                parametros.append("accion", 3);
                                parametros.append("cve_tipo_instructor", item.cve_tipo_instructor);
                                let {data,status} = await axios.post(ctr, parametros)
                                if(status == 200){
                                    if(data=="1"){
                                        fnConsultarTablaTipoInstructor();
            ...
            async function fnActivar(item){
                ...
                return{
                    ...
                    nombreTipoInstructor, dataTipoInstructor, headersTipoInstructor, 
                    fnConsultarTablaTipoInstructor, fnGuardarTipoInstructor, fnEliminarInstructor,
                    fnLimpiarCampos, fnActivar,
                    dialogBuscador, dialogDetallesCotizacion, dialogProveedor, searchTipos
