                    ...
                    <v-card>
                        ...
                        <v-container fluid>
                            ...
                            <v-row justify="center" class="align-center" style="padding: 0px 50px 0px 50px">
                                ...
                                <v-col md=4>
                                    <v-text-field v-model="nombreTipoEvento" outlined label="Nombre tipo evento"
                                        persistent-hint hint="Ingresa el nombre del tipo de evento"
                                        v-validate="'required|max:200'" data-vv-name="nombre tipo evento"
                                        :error="errors.has('nombre tipo evento')"
                                        :error-messages="errors.first('nombre tipo eventoo')"></v-text-field>
                                </v-col>
                                <v-row justify="center">
                                    <v-btn color="primary"
                                        @click="flagEditar ? fnEditar() : fnGuardarTipoEvento()"><v-icon>mdi-content-save</v-icon>{{flagEditar
                            ...
                            <v-row justify="center">
                                <v-col md=12>
                                    <v-data-table :headers="headersTipoEvento" :items="dataTipoEvento"
                                        :search="searchTipos" class="elevation-2"
                                        no-data-text="No se encontro ningun registro"
                                        :hide-default-header="dataTipoEvento.length < 1"
                                        :hide-default-footer="dataTipoEvento.length < 1" locale="es-ES"
                                        :mobile-breakpoint="NaN" items-per-page="10">

                                        <template v-slot:item.estatus="{item}">
                                            <v-chip style="width: 80px; justify-content: center;" class="ma-2" link
                                                @click="fnCambiarEstatus(item)"
                ...
                <%-- <v-dialog v-model="dialogBuscador" width="70%" scrollable transition="dialog-bottom-transition">
                    <v-card>
                        ...
                        <v-card-text>
                            ...
                            <v-row justify="center">
                                <v-col md=12>
                                    <v-data-table :headers="headersBusqueda" :items="dataBusqueda"
                                        :search="searchBusqueda" class="elevation-2"
                                        no-data-text="No se encontro ningun registro"
            ...
            <script type="module">
                ...
                new Vue({
                    ...
                    setup() {
                        ...
                        async function fnConsultarTablaTipoEvento() {
                            try {
                                preloader("../../");
                        ...
                        async function fnGuardarTipoEvento() {
