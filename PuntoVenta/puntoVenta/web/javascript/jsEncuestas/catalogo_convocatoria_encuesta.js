...
$dl.vPi = "../../../images/dload_estilo_1.gif";
	...
	function Activar(cve_convocatoria_encuesta_alumno)
	{
		var direccion = "ajax/proceso_convocatoria_encuesta.jsp?yCve_convocatoria_encuesta_alumno="+cve_convocatoria_encuesta_alumno+"&yAccion=Activar";
		...
		$jq.ajax({
			...
			success: function(data){
				...
				{
					$jq.dMsgUp("close")
					$jq.dMsgUp({mensaje: "upss! algo sali&oacute; mal al Validar."});
					$jq.dMsgUp("open");
				}
				else
				{
	...
	function Desactivar(cve_convocatoria_encuesta_alumno)
	{
		var direccion = "ajax/proceso_convocatoria_encuesta.jsp?yCve_convocatoria_encuesta_alumno="+cve_convocatoria_encuesta_alumno+"&yAccion=Desactivar";
	...
	{
		$jq.dMsgUp({mensaje: "Validando Fechas"});
		$jq.dMsgUp("open");
		
		var direccion = "ajax/proceso_convocatoria_encuesta.jsp?yInicio="+inicio.value+"&yFin="+fin.value+"&yAccion=Validar";
	...
	function Cargar(cve_convocatoria_encuesta_alumno)
	{
		//alert("Hola: " + cve_convocatoria_encuesta_alumno);
		document.forma.yAccion.value = "CargarDatos";
		document.forma.yCve_convocatoria_encuesta_alumno.value = cve_convocatoria_encuesta_alumno;
		document.forma.submit();
	}
	function Guardar()
	{
		var unidadA = document.getElementById("xUnidadAcademica");
		var descripcion = document.getElementById("xDescripcion");
		var inicio = document.getElementById("xFechaInicio");
		var fin = document.getElementById("xFechaFin");
		var activo = document.getElementById("xActivo");
		var action=document.getElementById("yTipoAccion").value;
		var yAccion="";
		var cve_convocatoria_encuesta_alumno=document.getElementById("yCve_convocatoria_encuesta_alumno").value;
		
		if(parseInt(action)==0){
			yAccion="Guardar";
			cve_convocatoria_encuesta_alumno=0;
		}else{
			yAccion="Actualizar";
		}
		...
		if(unidadA.value == 0)
		{
			$jq.dMsgUp({mensaje: "Debe selecionar una unidad academica"});
			$jq.dMsgUp("open");
			setTimeout(function(){$jq.dMsgUp("close");}, 3000);
		...
		{
			$jq.dMsgUp({mensaje: "Debe proporcionar una descripcion"});
			$jq.dMsgUp("open");
			setTimeout(function(){$jq.dMsgUp("close");}, 3000);
		...
		if(inicio.value == "")
		{
			$jq.dMsgUp({mensaje: "Debe seleccionar una fecha de inicio"});
			$jq.dMsgUp("open");
			setTimeout(function(){$jq.dMsgUp("close");}, 3000);
		...
		if(fin.value == "")
		{
			$jq.dMsgUp({mensaje: "Debe seleccionar una fecha de fin"});
			$jq.dMsgUp("open");
			setTimeout(function(){$jq.dMsgUp("close");}, 3000);
		...
		var direccion = "ajax/proceso_convocatoria_encuesta.jsp?yCve_convocatoria_encuesta_alumno="+cve_convocatoria_encuesta_alumno+"&yUnidad="+unidadA.value+"&yDescripcion="+descripcion.value+"&yInicio="+inicio.value+"&yFin="+fin.value+"&yActivo="+activo.checked+"&yAccion="+yAccion;
	...
	function Actualizar(cve_convocatoria_encuesta_alumno)
	{
		var unidadA = document.getElementById("xUnidadAcademica");
		var descripcion = document.getElementById("xDescripcion");
		var inicio = document.getElementById("xFechaInicio");
		...
		//calificacion, docente, cve_grupo_materia_frances, periodo, coordinador, parcial, matricula, motivo, nueva_calificacion, fecha
		var direccion = "ajax/proceso_convocatoria_encuesta.jsp?yCve_convocatoria_encuesta_alumno="+cve_convocatoria_encuesta_alumno+"&yUnidad="+unidadA.value+"&yDescripcion="+descripcion.value+"&yInicio="+inicio.value+"&yFin="+fin.value+"&yActivo="+activo.checked+"&yAccion=Actualizar";
		//alert("Liga: " + direccion);
		$jq.ajax({
			url: direccion,
			type: "post",
