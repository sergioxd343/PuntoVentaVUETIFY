        ...
        if(folioadmision.match(/^\d+$/)){//AAAA######AAAAAA##
            if($("#xFolio").val()!="" && eval($("#xFolio").val())>4){
                $.ajax({
                    ...
                    url:"../controlador/controlador_proceso_referencia_pago_nuevo_ingreso_acambaro_ing.jsp",
                    beforeSend:function(){
...
function CargarReferencia(cve_solicitud_admision){
    $("#xContenedor").load("../catalogo_referencia_pago_nuevo_ingreso_acambaro_ing.jsp",{xSolicitud:cve_solicitud_admision,cve_unidad_academica:7},function(){
        Ready();
