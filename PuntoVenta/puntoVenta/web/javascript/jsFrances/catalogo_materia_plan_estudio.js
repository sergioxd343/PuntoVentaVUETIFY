...
$dl.vPi = "../../../images/dload_estilo_1.gif";
	...
	function Guardar(cve_materia, unidad_academica, cve_plan, cuatrimestre)
	{
		var horas=document.getElementById("xHorasSemana");
		var faltas=document.getElementById("xFaltas");

		if(horas.value == 0)
		{
			$jq.dMsgUp({mensaje: "Debe proporcionar las horas asignadas"});
			$jq.dMsgUp("open");
		...
		if(faltas.value == 0.0)
		{
			$jq.dMsgUp({mensaje: "Debe proporcionar las faltas"});
			$jq.dMsgUp("open");
			setTimeout(function(){$jq.dMsgUp("close");}, 3000);
		...
		var direccion = "ajax/proceso_materia_plan_estudio.jsp?yUnidad="+unidad_academica+"&yHoras="+horas.value+"&yPlan="+cve_plan+"&yMateria="+cve_materia+"&yCuatrimestre="+cuatrimestre+"&yFaltas="+faltas.value+"&yAccion=Guardar";
